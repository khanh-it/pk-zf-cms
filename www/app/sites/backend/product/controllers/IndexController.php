 ...
 * @author khanhdtp
 */
class Product_IndexController extends K111_Controller_Action
{
	/**
	 * @var options
	 */
	...
	/**
	 * @var Product_Model_DbTable_Product
	 */ 
	protected $_repo;
	
	/**
	 * @var Product_Model_DbTable_Category
	 */ 
	protected $_repoCategory;
	
	/**
	 * @var Product_Model_DbTable_ProductCategory
	 */ 
	protected $_repoProductCategory;
	...
	public function init()
	{
		...
		//	throw new Zend_Controller_Action_Exception("Product type is missing!");
		//}
		...
		// +++ Entity's repo(s);
		// +++ +++ Product
		$this->_repo = new Product_Model_DbTable_Product();
		//$this->_repo->setDefaultType($this->_options['type']);
		// +++ +++ Category
		$this->_repoCategory = new Product_Model_DbTable_Category();
		$this->_repoCategory->setDefaultType(
			Product_Model_DbTable_Product::CATEGORY_TYPE_PRODUCT
		);
		// +++ +++ ProductCategory
		$this->_repoProductCategory = new Product_Model_DbTable_ProductCategory();
	}
	...
	public function indexAction()
	{
	    ...
	    // Define var # form;
	    $vData['form'] = $form = new Product_Form_Product_Index(array(
	        ...
	        if ($form->isValid($postData)) {
	            ...
	            if ($dataExists) {
		            $form->getElement('code')
						->addError($txt = $this->view->translate('Mã sản phẩm đã tồn tại!'))
		            ;
	            }
				// +++ Sku
	            ...
	            if (!$form->hasErrors()) {
					...
					$this->_repoProductCategory->insertProductCategory(
